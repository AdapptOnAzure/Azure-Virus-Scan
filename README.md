...
**File too large (413)**
```json
{"error": "File exceeds the maximum allowed size of 512 MB."}
```

# Limitations

- Microsoft Defender for Storage will not scan files larger than 50 GB. The maximum file size is configurable via `AV_MAX_FILE_SIZE_MB` (default: 512 MB) and is capped at 50 GB regardless of the configured value. In practice, the Azure Functions memory limit will be the real constraint for large files.
